...
// ---------------------------------------------------------------------------
const BASE_URL = 'http://103.151.63.84:8005';
...
const SPA_URL = 'https://iet-polinela.github.io/project-2026-nabhllaaa/';
...
const TEST_CITIZEN_PASSWORD = 'Citizen1234';
const TEST_ADMIN_USERNAME  = 'admin_test';
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    test('UI-02: Live Search pada daftar laporan admin berfungsi', async ({ page }) => {
        ...
        const tableBody = page.locator('#reportedTable');
        ...
        const responsePromise = page.waitForResponse(
            (response) => response.url().includes(`/dashboard/search/?q=${searchKeyword}`) && response.status() === 200,
            { timeout: 15000 }
        );
        ...
        const totalResults = (responseData.reported?.length || 0) + (responseData.resolved?.length || 0);
        if (totalResults > 0) {
    ...
    test('UI-03: Pagination Feed Kota — maks 10 kartu, kontrol pagination muncul', async ({ page }) => {
        ...
        const reportCards = listContainer.locator('article');
    ...
    test('UI-05: Isi form dan simpan draft → modal tutup, notifikasi muncul', async ({ page }) => {
        ...
        await page.locator('#reportCategory').fill('Infrastruktur');    // fill bukan selectOption
        await page.locator('#reportLocation').fill('Gedung Lab Analisis, Lantai 2');
        await page.locator('#reportDescription').fill('Unit AC di ruang Lab CPS 1 tidak berfungsi sejak tadi pagi. Suhu ruangan sangat panas dan mengganggu kegiatan praktikum.');
