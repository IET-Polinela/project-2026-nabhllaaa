...
# ─────────────────────────────────────────────────────────────────────────
    ...
    def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
        ...
        """
        # LANGKAH 1: Gunakan Django Client bawaan untuk mensimulasikan session-based authentication (Web Monolitik)
        from django.test import Client
        session_client = Client()

        # LANGKAH 2: Lakukan simulasi login menggunakan akun warga biasa (bukan staff/admin)
        session_client.login(username='warga_test', password='Password123!')

        # LANGKAH 3: Tentukan URL endpoint dashboard internal admin
        try:
            url = reverse('dashboard')
        except:
            # Fallback path jika penamaan name pattern route berbeda di urls.py
            url = '/dashboard/'

        # LANGKAH 4: Kirim GET request ke halaman dashboard admin tersebut
        response = session_client.get(url)

        # LANGKAH 5: Verifikasi sistem menolak akses dengan memberikan status HTTP 302 (Found/Redirect)
        # Sistem akan mengalihkan warga biasa menjauh dari halaman dashboard khusus petugas/admin
        self.assertEqual(
            response.status_code,
            status.HTTP_302_FOUND,
            "Warga biasa yang mencoba mengakses halaman admin harus dialihkan/diredirect (HTTP 302)"
        )