...
# ─────────────────────────────────────────────────────────────────────────
    ...
    def test_PRIV_03_tidak_bisa_baca_draf_orang_lain(self):
        ...
        """
        # LANGKAH 1: Autentikasi sesi API menggunakan Warga A
        self.client.force_authenticate(user=self.warga_a)

        # LANGKAH 2: Tentukan URL endpoint detail laporan berdasarkan ID draf milik Warga B
        url = f'/api/report/{self.draft_milik_b.id}/'

        # LANGKAH 3: Kirim GET request ke endpoint detail laporan tersebut
        response = self.client.get(url)

        # LANGKAH 4: Verifikasi status HTTP 404 Not Found dikembalikan demi menyembunyikan eksistensi draf
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Sistem harus menyembunyikan draf milik warga lain dengan mengembalikan HTTP 404 Not Found."
        )
    ...
    def test_PRIV_04_tidak_bisa_modifikasi_draf_orang_lain(self):
        ...
        # LANGKAH 3: Siapkan data payload baru untuk mencoba memodifikasi laporan secara paksa
        payload = {
            'title': 'Judul Diubah Paksa oleh Warga A',
            'category': 'Infrastruktur',
            'description': 'Mencoba meretas deskripsi laporan draf warga lain.',
            'location': 'Lokasi Eksploitasi',
            'status': 'DRAFT'
        }

        # LANGKAH 4: Kirim PUT request ke endpoint detail draf milik Warga B
        response = self.client.put(url, payload, format='json')

        # LANGKAH 5: Verifikasi sistem menolak operasi dengan mengembalikan HTTP 404 Not Found
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Sistem harus menolak operasi PUT pada draf pengguna lain dengan mengembalikan HTTP 404 Not Found."
        )